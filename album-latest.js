...
function renderAlbums(filterArtist = "All") {
  ...
  Object.keys(albumData).forEach(albumName => {
    ...
    if (["Radiant Brilliance","Goodcat Code","Valleydream Bloom","Passionate Appraisal","Innocent Birdcage","Alternative Darkborne","Abyssal Mark","Abyssal Blossom","Long-Awaited Reverly","Absurdity Unfolds","Midnight Stealth", "Magnum Opus", 
      "Night of Secrecy", "Where Heart Lives", "Lovespeed Ride","Improvised Flow","Primordial Chaos","Crimson Departure","Crimson Rapture","Darkbound Souls"].includes(albumName)) albumArtist = "Sylus";
    else if (["Absolute Zeal","Immediate Disorder","Runaway Waves","Diviner's Stillness","Diviner's Hymn","Aeon's Canon","Secret's Kiss" ].includes(albumName)) albumArtist = "Zayne";
    else if (["Mistsea Lament","Intertidal Zone","Fires of Devotion","Sweet Overdrive","Rippling Blossoms" ].includes(albumName)) albumArtist = "Rafayel";
    else if (["Between Stars", "Nightvow Requiem", "Nightvow Mooncurse","Misty Silhouette","Eulogy of Light" ].includes(albumName)) albumArtist = "Xavier";
    else if (["No-Return Night","Passionate Overload","Tainted Cuts"  ].includes(albumName)) albumArtist = "Caleb";
